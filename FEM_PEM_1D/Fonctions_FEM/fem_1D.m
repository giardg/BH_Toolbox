function [Hk, dHk, ddHk, xvec] = fem_1D( CF, xvec_init, Hk, dHk, P_init )
...
while iter < nbIter && max(critere_vec) > critere
    ...
    [U] = resoud_systeme( M, F, S, coord, bord_ess );
    
    %% Mise a jour de H et de sa derivee avec delta (H_{k+1} = H_k+delta)
    Hk_old = Hk;
    xvec_old = xvec;
    [Hk, dHk, ddHk, xvec, delta, flag] = update_champ(coord, connec, numer, xvec, Hk, U);
